...
router.post("/",async (req,res)=>{
    ...
    const {nome, sobrenome, idade, data_nasc, sexo,telefone} = req.body;
    
    //se algum dos campos não foi passado, irá enviar uma mensagem de erro ao retornar
    if(!nome || !sobrenome || !idade || !data_nasc || !sexo || !telefone){
        res.status(400).json({msg:"Enviar nome, sobrenome, idade, data_nasc, sexo e telefone do autor."});
        return;
    }
    ...
    try{
        ...
        const novo = await dbKnex("autores").insert({nome,sobrenome,idade,data_nasc,sexo,telefone});
...
//Filtro por nome ou por autor
router.get("/filtro/:palavra", async(req,res)=> {
    const palavra = req.params.palavra; // palavra ou nome a pesquisar
    try{
...
/*router.get("/dados/grafico",async (req,res) =>{
    try{
        ...
        const totalPorAno = await dbKnex("autores").select("idade")
        .sum({total:"data_nasc"}).groupBy("idade");
